...
class Emitter_PointTrails extends EmitterBase{
  ...
  scatterPoints( xy, count ){
      ...
      for(let x=0;x<count;++x){
        ...
        let tmp=new PointTrail(this.State, this.curId, pSeed, pointXY,pointVel,0,color,alpha,[],Math.floor(len), this.forces);
  ...
  genPoint( xy ){
    ...
    let tmp=new PointTrail(this.State, pCount, pSeed, [...xy],[Math.sin(pCount*230+pSeed+3434+len)*10,Math.cos(pCount*630+pSeed+134+len)*10],0,color,alpha,[],Math.floor(len), this.forces);
    this.points.push(tmp);
